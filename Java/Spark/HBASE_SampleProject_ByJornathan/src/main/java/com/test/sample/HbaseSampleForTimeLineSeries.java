...
public class HbaseSampleForTimeLineSeries {
...
//    public final static byte[] COL_PASSWORD = "password".getBytes();
	...
	public static String getTimeData(String strYear, String strMonth, String strDay, String strHour, String strMinute, String strSec){
		Calendar dateTemp = Calendar.getInstance();
		dateTemp.set(  Integer.parseInt(strYear),  Integer.parseInt(strMonth),  Integer.parseInt(strDay)
...
//                "hbase-site.xml"));
        ...
        try {
            ...
            String message = String.format("%s-%s-%s %s:%s:%s", strYear, strMonth, strDay, strHour, strMinute, strSec);
            
            String strYearMonthDayHour =  String.format("%s%s%s%s", strYear, strMonth, strDay, strHour);
            String strRowKey = String.format("%s%s%s%s%s", strYearMonthDayHour, "op01", strParameter, strYearMonthDayHour, "5959");
            
            String strStartDate =getTimeData(strYear, strMonth, strDay, strHour,  strMinute, strSec );
            String strEndDate = getTimeData(strYear, strMonth, strDay, strHour,  "59", "59" );
            
            //Put p = new Put(Bytes.toBytes("2015111709op1param0120151117095959"));  
            Put p = new Put(Bytes.toBytes(strRowKey));
            //p.addColumn(COLUMN_FAMILY, COL_NAME, Bytes.toBytes("2015111000op1param0120151110005959999"));            
            ...
            p.addColumn(COLUMN_FAMILY, COL_OPERATION, Bytes.toBytes("default"));
            p.addColumn(COLUMN_FAMILY, COL_TOOL, Bytes.toBytes("eqp01"));
            p.addColumn(COLUMN_FAMILY, COL_MODULE, Bytes.toBytes("chA"));
            p.addColumn(COLUMN_FAMILY, COL_START_DTTS, Bytes.toBytes(strStartDate)); //2015-12-16 15:00:00.000963
            p.addColumn(COLUMN_FAMILY, COL_END_DTTS, Bytes.toBytes(strEndDate)); //2015-12-16 15:59:59.000963
            p.addColumn(COLUMN_FAMILY, COL_PRODUCT, Bytes.toBytes("default"));
    		...
    		Calendar dateTemp = Calendar.getInstance();
    		dateTemp.set(  Integer.parseInt(strTimeData[0]),  Integer.parseInt(strTimeData[1]),  Integer.parseInt(strTimeData[2]),  Integer.parseInt(strTimeData[3]),  Integer.parseInt(strTimeData[4]),  Integer.parseInt(strTimeData[5]));
    		long customTimeValue = dateTemp.getTimeInMillis();
            ...
            for(int index = 0; index < 3600; index++){
            	int xVlue = index%20;
            	dateTemp.set(  Integer.parseInt(strTimeData[0]),  Integer.parseInt(strTimeData[1]),  Integer.parseInt(strTimeData[2]),  Integer.parseInt(strTimeData[3])
            			,  Integer.parseInt(strTimeData[4]),  index);
        		customTimeValue = dateTemp.getTimeInMillis();
        		
        		valueX.append(String.valueOf(xVlue) + "." + String.valueOf(xVlue) + ",");
...
//            valueY = valueY.substring(0, valueY.length()-2);
            ...
            p.addColumn(COLUMN_FAMILY, COL_VALUE, Bytes.toBytes(valueX.substring(0, valueX.length()-2)));
            p.addColumn(COLUMN_FAMILY, COL_TIME, Bytes.toBytes(valueY.substring(0, valueY.length()-2)));
