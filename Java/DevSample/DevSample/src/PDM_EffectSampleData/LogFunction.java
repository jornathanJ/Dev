...
public class LogFunction {
	...
	HashMap<Integer, double[]> tempValue = new HashMap<Integer, double[]>(); 
	...
	public HashMap<Integer, double[]> getEffectData() {
		...
		for (int index = 0; index < mTimeSequence.size() - 1; index++) {
			...
			tempValue = this.makeData((int) (timeGap), start.getTimeInMillis(), _StrTimeType);
			for(int loop = 0; loop < tempValue.size(); loop++ ){
				rtnValue.put(globalPosition++, tempValue.get(loop));
			}
		}
	...
	public HashMap<Integer, double[]> makeData(int sampleCnt, long startTime, String strType) {
		...
		for (double index = 0; index < sampleCnt; index += 1) {
			...
			/*System.out.printf("index is :%d, log10(%.2f) = %.3f [%d - %s] %n"
					, temp, index, result, resultTime, printLongTime(resultTime, false));*/

			rtnValue.put(temp, new double []{ resultTime, result});
