...
public class NewHbaseSampleForTimeDaySumFor {
	...
	public static void main(String[] args) {
		...
		try {
			...
			for (int loop = 0; loop < 24; loop++) {
				...
				String strRowKey = String.format("%s%s%s%s%s%s%s",
						strTool, strModule, strYear, strMonth , strDay, String.format("%02d", loop), strParameter);
				...
				String strValueMin = String.format("%.3f", 1 + 0.1*loop);
				String strValueMax = String.format("%.3f", 14 + 0.1*loop);
				String strValueSTD = String.format("%.3f", 5 + 0.1*loop + 0.01*loop);
				String strValueMEAN = String.format("%.3f", 11 + 0.1*loop);
