...
public class HourTraceSummaryPOC_PRESURE01 {
...
//
			...
			for (int loop = 0; loop < pressure01.min.length; loop ++)
			{
				...
				strMonth = String.format("%02d", startCalendar.get(Calendar.MONTH)+1);
...
//				p.addColumn(COLUMN_FAMILY, COL_MEAN, Bytes.toBytes(String.format("%f", _mean.mean_data[loop+initPosition])));
				...
				p.addColumn(COLUMN_FAMILY, COL_MIN, Bytes.toBytes(String.format("%f", pressure01.min[loop+initPosition])));
				p.addColumn(COLUMN_FAMILY, COL_MAX, Bytes.toBytes(String.format("%f", pressure01.max[loop+initPosition])));
				p.addColumn(COLUMN_FAMILY, COL_STD, Bytes.toBytes(String.format("%f", pressure01.std[loop+initPosition])));
				p.addColumn(COLUMN_FAMILY, COL_MEAN, Bytes.toBytes(String.format("%f", pressure01.mean[loop+initPosition])));
...
//								,_mean.mean_data[loop+initPosition]));
				...
				System.out.println(String.format("%s  [%f, %f, %f, %f]", strOutString
						, pressure01.min[loop+initPosition]
								, pressure01.max[loop+initPosition]
								, pressure01.std[loop+initPosition]
								,pressure01.mean[loop+initPosition]));
