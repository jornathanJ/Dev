...
public class HourTraceSummaryPOC_TEMP02 {
	...
	public static void main(String[] args) {
		...
		try {
			...
			String strSourceParameter = "trace.TEMP02";
...
//				p.addColumn(COLUMN_FAMILY, COL_MEAN, Bytes.toBytes(String.format("%f", _mean.mean_data[loop+initPosition])));
				...
				p.addColumn(COLUMN_FAMILY, COL_MIN, Bytes.toBytes(String.format("%f", temp02.min[loop+initPosition])));
				p.addColumn(COLUMN_FAMILY, COL_MAX, Bytes.toBytes(String.format("%f", temp02.max[loop+initPosition])));
				p.addColumn(COLUMN_FAMILY, COL_STD, Bytes.toBytes(String.format("%f", temp02.std[loop+initPosition])));
				p.addColumn(COLUMN_FAMILY, COL_MEAN, Bytes.toBytes(String.format("%f", temp02.mean[loop+initPosition])));
...
//								,_mean.mean_data[loop+initPosition]));
				...
				System.out.println(String.format("%s  [%f, %f, %f, %f]", strOutString
						, temp02.min[loop+initPosition]
								, temp02.max[loop+initPosition]
								, temp02.std[loop+initPosition]
								,temp02.mean[loop+initPosition]));
