...
public class HourTraceSummaryPOC_PRESSURE02 {
...
//
			...
			for (int loop = 0; loop < pressure02.min.length-1; loop ++)
			//for (int loop = 0; loop < 10; loop ++)
			{
				...
				startCalendar.add(Calendar.HOUR_OF_DAY, hourPosition++);
...
//				p.addColumn(COLUMN_FAMILY, COL_MEAN, Bytes.toBytes(String.format("%f", _mean.mean_data[loop+initPosition])));
				...
				p.addColumn(COLUMN_FAMILY, COL_MIN, Bytes.toBytes(String.format("%f", pressure02.min[loop])));
				p.addColumn(COLUMN_FAMILY, COL_MAX, Bytes.toBytes(String.format("%f", pressure02.max[loop])));
				p.addColumn(COLUMN_FAMILY, COL_STD, Bytes.toBytes(String.format("%f", pressure02.std[loop])));
				p.addColumn(COLUMN_FAMILY, COL_MEAN, Bytes.toBytes(String.format("%f", pressure02.mean[loop])));
